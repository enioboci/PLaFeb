...
let rec has_duplicate_fields : (string * 'a) list -> bool =
  fun fs ->
  match fs with
  | [] -> false
  | (id,_)::rest ->
    List.exists (fun (id',_) -> id = id') rest || has_duplicate_fields rest

let rec lookup_record_field : string -> (string * (bool * exp_val)) list -> exp_val ea_result =
  fun id fs ->
  match fs with
  ...
  | (name,(_,v))::rest ->
    if name = id
    then return v
    else lookup_record_field id rest

let rec string_of_tree t =
  match t with
  | Empty -> "Empty"
  | Node(v,l,r) ->
    "Node(" ^ string_of_expval v ^ "," ^ string_of_tree l ^ "," ^ string_of_tree r ^ ")"

and string_of_record_fields fs =
  String.concat "; "
    (List.map
       (fun (id,(_,v)) -> "(" ^ id ^ ",(" ^ "false" ^ "," ^ string_of_expval v ^ "))")
       fs)
...
and string_of_expval = function
  ...
  | RecordVal fs -> "RecordVal [" ^ string_of_record_fields fs ^ "]"
